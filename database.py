...
class EmailDB:
    def __init__(self,
        ...
        # Alternative DB
        self.single_table_alternative = single_table_alternative
        # SQLite DB
    ...
    def init_database(self):
        if self.single_table_alternative:
            self.cursor.execute('''
    ...
    def add_email_and_attachment(self, subject, sender, date, body, attach):
        attach_info = ''
        for file_name, file_data in attach:
            file_hash = hashlib.sha256(file_data).hexdigest()
            file_out = file_hash + pathlib.Path(file_name).suffix
            if not file_out in self.zip.namelist():
                self.zip.writestr(file_out, file_data)
            attach_info += f'{file_name}<=>{file_out}\n'

        self.cursor.execute('''
        INSERT INTO emails (subject, sender, date, body, attachments)
        VALUES (?, ?, ?, ?, ?)
        ''', (subject, sender, date, body, attach_info))
    ...
    def add(self, email):
        ...
        if self.single_table_alternative:
            self.add_email_and_attachment(mail_subject, mail_sender, mail_date, mail_body, mail_attach)
        else:
